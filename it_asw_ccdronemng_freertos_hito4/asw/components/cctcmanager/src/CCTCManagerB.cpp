	...
	EDROOM_CTX_Top_0::TEDROOMTransitionID arrivingTrans)
{
	...
	switch (arrivingTrans)
	{
		...
		case (EDROOM_CTX_Top_0::HandleTC_FwdHK_FDIRTC):
			...
			edroomNextState = edroomCurrentState;
			break;
		//From entry point HandleTC_FwdBKGTC
	...
	do
	{
		...
		switch(edroomNextState)
		{
			...
			case (InFlight):
				...
				//Go to the join point EvAction
			case (EvAction):
				//Arrival to join point EvAction
				edroomCurrentTrans=EDROOMEvActionArrival();
				break;
		...
		if (edroomCurrentTrans.distanceToContext == 0)
		{
			...
			switch (edroomCurrentTrans.localId)
			{
				...
				case (EvActionFromStanBy):
					//Go to the join point EvAction
					edroomNextState = EvAction;
					edroomContextExit=0;
					break;
...
TEDROOMTransId CCTCManager::EDROOM_SUB_Ready_1::EDROOMEvActionArrival()
{
	...
	do
	{
		...
		switch(Msg->signal)
		{
			...
			case (SEvAction): 
				 ...
				 {
					//Next transition is  EvActionFromStanBy
					edroomCurrentTrans.localId= EvActionFromStanBy;
					edroomCurrentTrans.distanceToContext = 0;
					edroomValidMsg=true;
				 }
		...
		switch(Msg->signal)
		{
			...
			case (SFlightFinished): 
				 ...
				 if (*Msg->GetPInterface() == DroneMngCtrl)
				{
					...
					//Next transition is  PlaneDone
					edroomCurrentTrans.localId= PlaneDone;
					edroomCurrentTrans.distanceToContext = 0;
					edroomValidMsg=true;
				 }
			...
			case (EDROOMIRQsignal): 
				 ...
				 if (*Msg->GetPInterface() == RxTC)
				{
					...
					//Next transition is  Bucle
					edroomCurrentTrans.localId= Bucle;
					edroomCurrentTrans.distanceToContext = 0;
					edroomValidMsg=true;
				 }
			...
			case (SEvAction): 
				 ...
				 {
					//Next transition is  EvActionFromFlight
					edroomCurrentTrans.localId= EvActionFromFlight;
					edroomCurrentTrans.distanceToContext = 0;
					edroomValidMsg=true;
				 }
